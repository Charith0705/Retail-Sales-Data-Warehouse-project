...
CREATE TABLE IF NOT EXISTS sales_dwh.silver.dim_customer (CustomerSK BIGINT GENERATED ALWAYS AS IDENTITY,CustomerID INT,CustomerName STRING,Email STRING,City STRING,Address STRING,StartDate DATE,EndDate DATE,IsActive INT)USING DELTA
...
USING (
  SELECT
    CustomerID,TRIM(INITCAP(CustomerName)) AS CustomerName,LOWER(TRIM(Email)) AS Email,TRIM(City) AS City,TRIM(Address) AS Address
  FROM (
    SELECT *,
...
WHERE rn = 1
AND NOT EXISTS (
  SELECT 1 FROM sales_dwh.silver.dim_customer tgt WHERE tgt.CustomerID = src.CustomerID AND tgt.IsActive = 1
);
...
USING (
  SELECT
    s.TransactionID,dc.CustomerSK,dp.ProductSK,ds.StoreSK,
...
-- VALIDATION 
SELECT 'dim_customer' AS table_name, COUNT(*) AS total_rows, SUM(CASE WHEN IsActive = 1 THEN 1 ELSE 0 END) AS active_records
