...
AS
SELECT
  CustomerSK,CustomerID,CustomerName,Email,City,Address,StartDate,EndDate,IsActive
...
AS
SELECT
  SalesSK,TransactionID,CustomerSK,ProductSK,StoreSK,Quantity,Amount,TxnDate
FROM sales_dwh.silver.fact_sales;

--FINAL VALIDATION
SELECT 'dim_customer' AS table_name, COUNT(*) AS total_rows, SUM(CASE WHEN IsActive = 1 THEN 1 ELSE 0 END) AS active_rows
