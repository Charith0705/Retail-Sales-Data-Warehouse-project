...
USING (
  SELECT
    CAST(CustomerID AS INT) AS CustomerID, CustomerName, Email, City, Address, LastUpdated,current_timestamp() AS ingested_at
  FROM read_files(
...
USING (
  SELECT
    CAST(ProductID AS INT) AS ProductID,ProductName,Category,CAST(UnitPrice AS DECIMAL(10,2)) AS UnitPrice,current_timestamp() AS ingested_at
  FROM read_files(
...
USING (
  SELECT
    CAST(StoreID AS INT) AS StoreID,StoreName,Region,current_timestamp() AS ingested_at
  FROM read_files(
...
USING (
  SELECT
    CAST(TransactionID AS INT) AS TransactionID,CAST(CustomerID AS INT) AS CustomerID,CAST(ProductID AS INT) AS ProductID,CAST(StoreID AS INT) AS StoreID,CAST(Quantity AS INT) AS Quantity,TxnDate,current_timestamp() AS ingested_at
  FROM read_files(
