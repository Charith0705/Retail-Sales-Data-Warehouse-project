...
SELECT CASE
  ...
  ELSE 'raw_customers has ' || CAST(COUNT(*) AS STRING) || ' rows'
...
SELECT CASE
  ...
  ELSE 'raw_products has ' || CAST(COUNT(*) AS STRING) || ' rows'
...
SELECT CASE
  ...
  ELSE 'raw_stores has ' || CAST(COUNT(*) AS STRING) || ' rows'
...
SELECT CASE
  ...
  ELSE 'raw_sales has ' || CAST(COUNT(*) AS STRING) || ' rows'
...
SELECT CASE
  ...
  ELSE 'No null CustomerIDs in raw_customers'
...
SELECT CASE
  ...
  ELSE 'No null ProductIDs in raw_products'
...
SELECT CASE
  ...
  ELSE 'No null StoreIDs in raw_stores'
...
SELECT CASE
  ...
  ELSE 'No null TransactionIDs in raw_sales'
...
SELECT CASE
  ...
  ELSE 'ingested_at populated in all Bronze tables'
